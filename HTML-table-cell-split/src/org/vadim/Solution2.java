...
public class Solution2 {
	...
	private static void makeSplitColumns(final ArrayList<List<ICell>> rows, int rowIdx, int colIdx) {
		...
		// split the target
		Cell cell = (Cell) rows.get(rowIdx).get(colIdx);
		Cell newCell = new Cell(rowIdx, colIdx+1, cell.cspan, cell.rspan);
		RefCell ref = new RefCell(newCell);
		
		for (int y = rowIdx; y < rowIdx + cell.rspan; y++) {
			for (int x = colIdx + 1; x < colIdx + cell.cspan + 1; x++) {
				rows.get(y).set(x, ref);
			}
		}

		rows.get(rowIdx).set(colIdx + 1, newCell);
	}
	...
	private static void makeSplitRows(final ArrayList<List<ICell>> rows, int rowIdx, int colIdx) {
		...
		// split the target
		Cell cell = (Cell) rows.get(rowIdx).get(colIdx);
		Cell newCell = new Cell(rowIdx+1, colIdx, cell.cspan, cell.rspan);
		RefCell ref = new RefCell(newCell);
		
		for (int y = rowIdx + 1; y < rowIdx + cell.rspan + 1; y++) {
			for (int x = colIdx; x < colIdx + cell.cspan; x++) {
				rows.get(y).set(x, ref);
			}
		}

		rows.get(rowIdx + 1).set(colIdx, newCell);
	}
	...
	public static class RefCell implements ICell {
		...
		public String toString() {return String.format("REF(%d,%d,%d,%d)", cell.x, cell.y, cell.cspan, cell.rspan);}
	}
	public static class Cell implements ICell {
		...
		public String toString() {return String.format("(%d,%d,%d,%d)", x, y, cspan, rspan);}
	}
