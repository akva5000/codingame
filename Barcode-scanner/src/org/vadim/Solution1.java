...
public class Solution1 {
	...
	public static void main(String args[]) throws Exception {
		...
		for (int pos = 3; pos < 45; pos += 7) {
			Callable<Boolean> task = new ZoneABHandler(scanline, pos, code, codePos++, prefix);
			if (!ForkJoinPool.commonPool().submit(task).get()) {
				System.err.println("error for digit, " + (codePos - 1));
				System.out.println("INVALID SCAN");
				return;
			}
		}
		...
		for (int pos = 50; pos < 92; pos += 7) {
			Callable<Boolean> task = new ZoneCHandler(scanline, pos, code, codePos++);
			if (!ForkJoinPool.commonPool().submit(task).get()) {
				System.err.println("error for digit, " + (codePos - 1));
				System.out.println("INVALID SCAN");
				return;
		...
		Integer prefixDigit = PATTERN.get(String.valueOf(prefix));
		if (prefixDigit == null) {
			System.out.println("INVALID SCAN");
			return;
		}
